...
async function connect() {
  ...
  onValue(unsubVtt, snap => {
    vtt = snap.val() || { scene: { mapUrl: '', gridSize: 48 }, tokens: {} };
...
function addMonster() {
  ...
  writeSet(vttStatePath(`tokens/${monsterId()}`), token, 'Не удалось добавить монстра')
...
function removeToken(id) {
  ...
  writeRemove(vttStatePath(`tokens/${id}`), 'Не удалось удалить токен');
}
...
function updateTokenHp(id, delta) {
  ...
  if (token.type === 'monster') {
    writeSet(path, Object.assign({}, token, { hpCur }), 'Не удалось изменить ХП');
  } else {
...
function tokenWithLiveData(id, token) {
  if (token.type !== 'player') return Object.assign({}, token, pendingTokenPatches[id]?.patch || {});
  const data = players[token.playerKey] || {};
  const summary = playerSummary(data, token.name || token.playerKey);
  return Object.assign({}, token, summary, pendingTokenPatches[id]?.patch || {});
}
...
function renderTokenInspector() {
  ...
  const data = tokenWithLiveData(selectedTokenId, token);
  ...
  box.innerHTML = `
    ...
    <textarea id="tokenNoteInput" class="inspector-note" placeholder="Заметка мастера или игрока..." ${canEdit ? '' : 'disabled'}>${escapeHtml(token.note || '')}</textarea>
...
function saveSelectedToken() {
  ...
  if (isMonster) patch.name = el('tokenNameInput').value.trim() || token.name || 'Монстр';
  if (isMonster || canMoveToken(token)) patch.note = el('tokenNoteInput').value.trim();

  const writePromise = isMonster
    ? saveMonsterToken(selectedTokenId, token, patch)
    : savePlayerTokenStats(token, patch);
  renderBoard();
  renderPlayers();
  renderMonsters();
  return writePromise
...
function saveMonsterToken(id, token, patch) {
  ...
  return writeSet(vttStatePath(`tokens/${id}`), fullToken, 'Не удалось сохранить токен');
}

function savePlayerTokenStats(token, patch) {
  ...
  Object.assign(players[token.playerKey], playerPatch);
  rememberPendingTokenPatch(selectedTokenId, patch);
  const writes = [
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'note')) {
    ...
    writes.push(writeUpdate(vttStatePath(`tokens/${selectedTokenId}`), { note: patch.note }, 'Не удалось сохранить заметку'));
  }
...
function renderMonsters() {
  ...
  list.innerHTML = monsters.map(([id, token]) => {
    const color = hpColor(token.hpCur, token.hpMax);
    const pct = Math.max(0, Math.min(100, Math.round((token.hpCur || 0) / (token.hpMax || 1) * 100)));
    return `
      <div class="card">
        <div class="card-head">
          <div>
            <div class="card-name">${escapeHtml(token.name)}</div>
            <div class="card-meta">КД ${escapeHtml(token.ac)} · ${escapeHtml(token.hpCur)} / ${escapeHtml(token.hpMax)} ХП</div>
          </div>
